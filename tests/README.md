...
- `host-session-affinity.integration.test.ts` 通过 `DefaultResourceLoader` + `AgentSession.bindExtensions` 加载真实扩展，验证会话亲和从 Custom Entry 持久化/恢复；挂载方式见 [`../docs/decisions/0010-host-level-extension-e2e-harness.md`](../docs/decisions/0010-host-level-extension-e2e-harness.md)。
